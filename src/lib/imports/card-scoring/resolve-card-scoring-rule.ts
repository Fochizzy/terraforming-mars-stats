...
export async function resolveCardScoringRule(input: {
  ...
  if (!ocrTextLines) {
    try {
      ocrTextLines = input.readOcrTextLines
        ? await input.readOcrTextLines(input.card)
        : await readOcrTextLinesFromUrl(input.card.fullImageUrl);
    } catch {
      return {
  ...
  let parsedRule: ReturnType<typeof parseOcrCardRule>;

  try {
    parsedRule = parseOcrCardRule({
      cardName: input.card.cardName,
      textLines: ocrTextLines,
    });
  } catch (error) {
    console.warn('Card scoring OCR parse failed', {
    ...
    return {
      reason: `Unable to parse ${input.card.cardName} for OCR rule scoring.`,
      status: 'review',
    };
  }
