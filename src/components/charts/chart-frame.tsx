  ...
  if (terraformingShare >= 34) {
    return {
      chips: [
        "Fast tempo",
        "TR-led scoring",
        boardShare >= 15 ? "Board support" : "Lean board",
      ],
  ...
  if (engineShare >= 44) {
    return {
      chips: [
        "Card economy",
        "Engine payoff",
        objectiveShare >= 10 ? "Objective support" : "Low objectives",
      ],
  ...
  if (boardShare >= 22) {
    return {
      chips: [
        "Map pressure",
        "Tile scoring",
        terraformingShare >= 25 ? "Strong tempo" : "Patient tempo",
      ],
      ...
      <div className={scoreStyles.header}>
        <div>
          ...
          <p className={scoreStyles.description}>
            See which scoring lanes build the average final score and what that
            mix says about the way {focusedName ?? "this group"} plays.
          </p>
        </div>
        ...
        <>
          <div className={scoreStyles.metrics}>
            <div className={scoreStyles.metric}>
              ...
              <strong className={scoreStyles.metricValue}>
                {formatPoints(total)} pts
              </strong>
              <span className={scoreStyles.metricNote}>
                Sum of recorded score sources
              </span>
            </div>
            <div className={scoreStyles.metric}>
              ...
              <span className={scoreStyles.metricNote}>
                {formatPoints(dominant.value)} pts ·{" "}
                {Math.round(dominant.share)}% of score
              </span>
            </div>
            <div className={scoreStyles.metric}>
              ...
              <span className={scoreStyles.metricNote}>
                Based on the current scoring mix
              </span>
            </div>
          </div>
          ...
          <div className={scoreStyles.contentGrid}>
                ...
                return (
                  ...
                  >
                    <div className={scoreStyles.cardTop}>
                      ...
                      <span className={scoreStyles.cardRank}>
                        #{category.rank}
                      </span>
                    </div>
                    <h3 className={scoreStyles.cardTitle}>{category.label}</h3>
                    <p className={scoreStyles.cardValue}>
                      {formatPoints(category.value)} pts
                    </p>
            ...
            <aside className={scoreStyles.insight}>
              <div>
                <p className={scoreStyles.insightLabel}>
...
export function ChartFrame({
  ...
  if (
    title === "Group Score Profile" ||
    title.startsWith("Score Profile for ")
  ) {
