-- Count preludes gained during the game (Board of Directors, Valley Trust, New
-- Partner) anywhere the app reports prelude selection stats. They are still
-- stored separately so the review flow can distinguish setup picks from
-- in-game picks, but analytics should list both groups under preludes.
comment on table public.game_player_midgame_preludes is
  'Preludes a player played after the opening prelude phase (Valley Trust, Board of Directors, New Partner). Stored separately for review, but included in prelude selection analytics.';
...
prelude_entries as (
  select gpp.game_player_id, gpp.prelude_id
  from game_player_preludes gpp

  union

  select gpmp.game_player_id, gpmp.prelude_id
  from game_player_midgame_preludes gpmp
),
prelude_base as (
  select e.*, pre.name as prelude_name
  from enriched e
  join prelude_entries pe on pe.game_player_id = e.id
