-- Extend contextual card outcomes with game pace, table size, and map. This is
-- forward-only because the initial context view has already reached production.
...
left join public.maps m on m.id = g.map_id
