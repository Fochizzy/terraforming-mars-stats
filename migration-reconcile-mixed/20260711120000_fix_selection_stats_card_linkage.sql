-- Fix the `cards` payload of get_selection_stats.
--
-- The card_plays CTE joined game_log_events on gle.game_player_id = e.id, but
-- game_log_events.game_player_id is never populated by the import pipeline (the
-- player is carried in payload->>'actor'). That join matched zero rows, so the
-- `cards` field was always an empty array. Re-source card plays from
-- analytics.game_log_event_facts, which resolves player_id from the log actor
-- the same way the Insights card-outcome view does, joining to the enriched
-- results by (game_id, player_id). Only this one CTE changes.
