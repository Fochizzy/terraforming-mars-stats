-- Style effectiveness, aggregated by inferred play style, for three scopes:
--   'global'   — every finalized game (all players, all groups)
--   'personal' — only games the signed-in user's linked players appear in
--   'group'    — a single group (p_group_id)
--
-- SECURITY DEFINER so 'global' can read across every group's rows (the analytics
-- views are security_invoker and would otherwise be RLS-scoped to the caller's
-- own groups). 'personal' is still scoped to auth.uid()'s players, and 'group'
-- to the requested group, so a caller can't see more than the scope they ask
-- for beyond the intentionally public global aggregate. Returns the same shape
-- as the group_style_performance / *_score_source_averages views so the app can
-- reuse its existing mappers.
    ...
    where
      case
        ...
        when p_scope = 'group' then
          ...
          and pgr.group_id = p_group_id
          and (select public.is_group_member(p_group_id))
        else true
      end
  ),
  styles as (
