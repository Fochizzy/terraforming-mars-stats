-- Expands selection statistics with placement distributions, award-funding
-- ROI, card-level win correlation, and tag/win correlation, plus a
-- group-scoped head-to-head function.
...
corp_stats as (
  select
    ...
    round(avg(e.placement)::numeric, 2) as avg_placement,
    count(*) filter (where e.placement = 1) as first_place_finishes,
    count(*) filter (where e.placement = 2) as second_place_finishes,
    count(*) filter (where e.placement >= 3) as third_plus_finishes,
...
tag_win_stats as (
  select
    ts.tag_code,
    round(avg(ts.tag_count) filter (where e.is_winner)::numeric, 1) as avg_tags_in_wins,
    round(avg(ts.tag_count) filter (where not e.is_winner)::numeric, 1) as avg_tags_in_losses,
    count(*) as samples
  from enriched e
  join players p on p.id = e.player_id
  join game_log_imports gli on gli.game_id = e.game_id
...
-- Head-to-head records within a group. SECURITY INVOKER: row-level security
-- limits results to groups the caller belongs to.
...
corp_matchups as (
  ...
  join corporations ca on ca.id = a.corporation_id
  join corporations cb on cb.id = b.corporation_id
