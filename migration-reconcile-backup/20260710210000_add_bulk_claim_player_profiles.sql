-- A person plays across several groups, and each group carries its own
-- per-roster `players` row (see the group-collapse split). Claiming one profile
-- at a time forced them to click through every group. This links every unclaimed
-- roster entry whose name exactly matches the signed-in account in one action.
...
begin
  ...
  -- Locking + capturing the claimed rows up front keeps the group-member and
  -- profile follow-up writes aligned with exactly what the update touched.
