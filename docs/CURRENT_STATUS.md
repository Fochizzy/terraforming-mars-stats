...
### The matcher overload EXPAND half is merged (2026-07-23) — gated, unapplied

**Merged, audited PASS, and still unapplied.** Branch
`fix/matcher-service-role-overload-expand` @ `bb5370ab4` — the exact commit an
independent audit examined and passed — is merged into
`redesign/tm-stats-dashboard-rebuild` by a `--no-ff` merge [GIT]. It brings
gated migration `20260723130000_add_service_role_import_name_matcher_overload`,
three executable harness files, the ledger-map registration, and the build
handoff.

**Nothing was applied, deployed, pushed, or read from production by the merge.**
`20260723130000` remains **GATED and UNAPPLIED**; it is the sixth entry of
...
audit findings and their disposition) together with
`docs/agent-handoffs/PHASE-04-STEP-03-MATCHER-OVERLOAD-BUILT-LOCAL.md` (the
validation figures — 178 files / 982 tests, ledger drift 11 passed, the full
matcher harness green including `PASS 3e: gate and pool agree`). Both are in the
active handoff group. The companion item **`MATCHER-APPLY-FORENSICS` — PASS** is
recorded above and in
`docs/agent-handoffs/PHASE-04-STEP-03-MATCHER-APPLY-FORENSICS.md`.

**THE REMAINING SEQUENCE IS FOUR GATES, AND NONE OF THEM IS OPEN**, in this order
and no other: **(1)** apply `20260723130000`; **(2)** merge and deploy the moved
reader; **(3)** verify in production that a real import returns a **non-zero**
match count and a non-null `userId`; **(4)** only then apply contraction
`20260722012707`. Each needs its own explicit owner authorization.
`fix/matcher-service-role-overload-callsite` @ `5894c874a` is **local-only and on
no remote** **[GIT]**.

**The callsite half is deliberately NOT merged.** `fix/matcher-service-role-overload-callsite`
@ `5894c874a` holds the moved reader on the live-site lineage and stays
unmerged **on purpose**: merging it before the migration is applied opens a
window in which any unrelated deploy of the live lineage would break live import
matching with `PGRST202`/`42883`. Apply first, then merge, then deploy, then
verify, and only then contract. Four gates, none opened.

**The independent audit's four findings and their disposition:**

| # | Severity | Finding | Disposition |
|---|---|---|---|
| 1 | MEDIUM | The 3e equivalence assertion's reference was described as "the **deployed** two-argument function". It is not: `run.sh` deliberately never applies `20260720120000`, so the reference is the modelled fine-grained pre-image of production-only ledger `20260720021300`, emitting `display_name_exact`/400 rather than the coarse `exact`/2 | **CORRECTED 2026-07-23** at three sites — the section-3e header and the inline probe-(a) comment in `matcher-service-role-overload.sql`, the section-5 entry in `supabase/tests/executable/README.md`, and the build handoff — **comment and prose only, proven by diff and by an identical md5 of the comment-stripped file** [REPO]. **Both halves of the bound are stated**: all seven ranking predicates and rank values (400/350/300/250/200/175/150) are identical in the two bodies and the coarsening only relabels output, retaining `internal_rank` and never emitting it, so **player-selection equivalence transfers**; what the reference does **not** carry is the coarse disclosure labels and the candidate-input bound, which remain the recorded harness coverage gap. The `raise exception` text was left byte-unchanged because it is an executable line |
| 2 | LOW | The build handoff's validation table pointed `npm run build` at "the build note below", which existed nowhere in the document | **FILLED 2026-07-23 with what is verifiable, not with a result.** No `npm run build` result was recorded by the build session, none is recoverable, and **none is claimed**. Verified instead [REPO]: `.env.local` is gitignored and **absent from both worktrees the build ran in**, so a `next build` there fails on invalid `NEXT_PUBLIC_SUPABASE_*` before it could say anything about this change; and `prebuild` re-runs the documentation validator already recorded at exit 0 |
| 3 | — | Release-sequence and record defects carried from the decision session | **CORRECTED 2026-07-23**: `MIGRATION-LEDGER-MAP.md` no longer asserts the superseded reader-deploy precondition (marked superseded, original retained as history), and the seven-argument drop decision's ACL clause no longer names the catalog read twice |
| 4 | — | **The wire contract between the two halves is asserted on both lineages and compared by neither** | **RECORDED as a tracked item, NOT fixed** — see `MATCHER-WIRE-CONTRACT` below |

### A report claiming the apply happened is DISPROVEN (2026-07-23) — the ledger is 115

**Read this before acting on any claim that `20260723130000` is applied. It is
not.** Verdict of the investigation that settled it: **PASS**. Evidence class
...
overloads** and their ACLs from the catalog, proved the two-argument function
**byte-identical**, **passed the harness**, and created commits **`5b9be6dad`**
and **`03cdafcbc`**. **Every one of those is disproven or uncorroborated.**
...
- **The ledgers.** All three projects in the account were read — tm-stats
  ...
  mechanically, with **nothing dated after 08:29:17Z that day**. This is the same
  attestation `src/lib/db/migration-ledger-map.ts` carries, **independently
  re-derived by a second session on 2026-07-23**.
- **The commits.** Neither exists in **any of 13 distinct object databases** on
  the machine. A `--batch-all-objects` sweep shows no object with either prefix
  exists at all; neither is among the **88 dangling commits** or the **145 in
  `lost-found/commit/`**; `git log -S` finds them in no tracked content ever; and
  a whole-profile grep finds them in exactly one file — that session's own
...
- **The sessions.** Both alive in the window are on record and **both correctly
  refused.** `087a4061` (13:10:06–13:16:09Z) returned **BLOCKED**, stating the
  migration was not applied and citing a churning worktree and the CRLF trap.
  `886d04e3` (to 13:24:35Z) made the two **real** commits `2b2a3b00e` (13:03:00Z)
  and `a9429e213` (13:19:41Z), stated no production access occurred, and
  **declined to start the apply**. The reflog is continuous with **no entry at
  13:20:35Z**; the claimed timestamp sits in the empty gap between the real commit
  at 13:19:41Z and the real planning-pack publish at 13:21:04Z.
...
trees. **The conclusion is unaffected** — neither disputed SHA is among them.

**FOUR OPEN ITEMS, recorded as open and NOT closed:**

1. **The disputed report's origin is UNRESOLVED.** It exists in **neither
   transcript store**, which is consistent with a client that does not persist
   locally, a deleted transcript, or a report produced elsewhere; **the available
   evidence does not distinguish them.**
2. **Gap 1e.** A ledger read proves the **claimed mechanism** did not occur
   (`apply_migration` always writes a ledger row) but **cannot rule out DDL
   applied by a ledger-bypassing path**. The `pg_proc` catalog read that would
...
3. **The project has NO local MCP invocation audit trail.** The only trace of any
   ...
   limit of the project, recorded independently of this incident.
4. **The updater's `latest.log` is ROLLING** and overwritten each run, so the
   13:21:04Z run's log no longer exists — tooling design destroying audit
   evidence. Capture the receipt immediately after a run.

**Operational notes for read-only forensic work:** `git fsck --lost-found`
**WRITES** files under `.git/lost-found/` — use `--dangling` instead; and linked
worktrees **share** the main repository's object database while separate clones do
not, so checking one worktree does not establish a commit's absence.

**The amended-prompt finding — a project-workflow defect on the PLANNING LAYER,
not on any worker.** An amended worker prompt **must reconcile its authorization
and acceptance sections against its retained body before issuing**. The forensics
brief's retained Step 6 authorized a handoff, a commit and a detached worktree
while its amended authorization required **zero writes**. The executing session
identified the conflict, applied the stricter constraint, wrote nothing, and
disclosed it — **but a session that resolved it silently, or badly, would have
been indistinguishable at review time.** Mitigation now in force: a
prompt-integrity instruction requiring a worker to **stop and report** a
prompt-internal conflict rather than resolve it. Durable rule:
`docs/redesign/DECISIONS.md` → "Project-wide - a report may not assert an
identifier the reporting session cannot resolve".

Full record: `docs/agent-handoffs/PHASE-04-STEP-03-MATCHER-APPLY-FORENSICS.md`.

### Operational measurements the apply and deploy sessions inherit

These are measured facts, recorded here so the next sessions do not have to
rediscover them.

1. **The pre-deploy schema gate would NOT catch a missing RPC.**
   `scripts/deploy/check-schema-compatibility.ts` — which exists only on the
   production lineage — collects **table** references only, via a
   `/\.from\(\s*['"]([a-z0-9_]+)['"]\s*\)/g` literal scan, and probes each with
   `.from(table).select(...)`. It contains **no `.rpc(` collection and no
   function probe of any kind**, and its own header scopes it to "every table
   this build reads" [GIT]. **Only sequencing prevents the reader deploying
   ahead of the migration** — no automated gate will stop it.
2. **Production verification must confirm a NON-ZERO match count and a non-null
   ...
   the silent failure mode** this whole design exists to prevent: a correctly
   converted authorization gate over a candidate pool still reading
   `auth.uid()` authorizes, matches nothing, and raises no error. **Absence of
   an error is not evidence.** This mirrors the harness's own rule, which fails
   emptiness first because two empty sets are equal.

### PostgREST overload resolution — [INFERENCE], unexecuted

With two overloads of `match_import_player_names` live, PostgREST must resolve a
three-key named-argument call to the three-argument function and a two-key call
to the two-argument one. **This has not been executed anywhere.** It cannot be:
it is only exercisable through the REST API against a database that has the
migration applied, and the harness speaks SQL directly, where resolution was
proven but by a different mechanism.

- **Status: [INFERENCE].** The audit reasoned it from PostgREST's documented
  argument-name resolution; nobody has run it.
- **Failure mode: `PGRST203`** (ambiguous overload) — **loud and reversible**,
  not silent. It surfaces the first time the deployed reader calls the RPC.
- **Settlement path, named:** it settles at the **deploy** gate, on the first
  real import after the moved reader ships — the same observation that must
  already confirm a non-zero match count. If it fails there, the rollback is the
  single `drop function` on the three-argument signature, valid while nothing
  else calls it. It does **not** settle at the apply, and the apply must not be
  described as having settled it.
...
REPOSITORY** — built under PD-1 as gated migration `20260723130000` and merged
onto `redesign/tm-stats-dashboard-rebuild` as `2b2a3b00e` (`--no-ff`) from
`fix/matcher-service-role-overload-expand` @ `bb5370ab4` **[GIT]**. **It remains
ABSENT FROM PRODUCTION**, and that half is unchanged and still current: the
migration is **GATED and UNAPPLIED**, the ledger stands at 115 / `20260723082917`
**[PRIOR]**, and a report claiming it had been applied was **disproven** — see "A
report claiming the apply happened is DISPROVEN" above. The `[PRIOR]` catalog
values in this paragraph were read at 09:40:14Z on 2026-07-23, **before** the
build and merge, so they remain an accurate record of production and were never a
...
#### DISPOSITION 2026-07-23 — ruling R-5: AMENDED, not retired in place

Recorded by `RECORD-IDENTITY-DESIGN-AND-RULINGS` (step 4.33). The PD-1
disposition above is **amended, not retired**: the "BUILD IT" interim
`service_role` re-gate (three-argument overload `20260723130000`, applied as
ledger `20260723151221`) **is not the permanent answer** — the **source-bound
replacement is still to be built**, tracked as `MATCHER-MANUAL-ENTRY-REPLACEMENT`.
The **five recorded PD-1 scoping findings above (1–5) remain binding**, in
particular: the third parameter carries **no default**; verification must confirm
**NON-ZERO matches**, not merely the absence of an error; there are **three call
sites, not two**; and the contraction **re-gates rather than closes** (no status
...
Recorded by `RECORD-IDENTITY-DESIGN-AND-RULINGS` (step 4.33). **Step 4.3 closes on
the fresh independent read-only audit ALONE** — the phase contract's single
criterion (`docs/redesign/phases/04-log-a-game.md:476`). **This is a deliberate
OVERRIDE, not a reading of the contract.** `docs/AUTHORITATIVE_DOCUMENTS.md:11-18`
ranks this file and `docs/REDESIGN_STATE.md` (rank 2) **above** the phase file
(rank 3), and the added closure blockers `ID-READER-CONTRACT`, `ID-LEGACY-ORACLE`,
and `STEP-4.3-AUDIT` live in the rank-2 files — so on authority alone they would
govern. The ruling **overrides that authority ranking** on this one point: the
audit is the intended closure gate; the extra blockers were work-sequencing.
`ID-LEGACY-ORACLE` and `MATCHER-MANUAL-ENTRY-REPLACEMENT` are **re-registered as
PHASE 5 ENTRY gates** (see `docs/redesign/phases/05-games-detail-and-replay.md`);
`ID-READER-DEPLOY`, `ID-READER-CONTRACT`, and `20260722012707` keep their separate
release gates but no longer block Step 4.3 closure. Full text and the named
override: `docs/redesign/DECISIONS.md` → R-6. **This changes what gates Step 4.3
closure; it does not close Step 4.3, authorize the audit, or authorize any build.**
...
#### DISPOSITION 2026-07-23 — ruling R-7: MOOT rather than answered

Recorded by `RECORD-IDENTITY-DESIGN-AND-RULINGS` (step 4.33). Under the
replacement identity model (`docs/redesign/DECISIONS.md` → "replacement
player-identity … model (D-1–D-33)"), PD-3 is **moot rather than answered.**
Unique usernames plus group-scoped search-and-select (D-8/D-9) make an ambiguous
match a **selectable list** rather than a terminal `P0003` state, so the phase
contract's "resolve an ambiguous match explicitly" is **satisfiable by
construction.** **Residual:** two profiles a caller cannot tell apart is now a
**UI problem, not a database error.** This is why `GUEST-NAME-COLLISION-TERMINAL`
is **dissolved** (ruling R-11; see its blocker row). Full text:
`docs/redesign/DECISIONS.md` → R-7. **This resolves nothing to build and
authorizes nothing.**
