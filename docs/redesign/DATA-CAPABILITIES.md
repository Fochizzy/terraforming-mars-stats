...
| Overall point differential | Requires further verification | analytics.player_game_results.signed_differential currently means distance to the adjacent placement; persisted win/loss gaps exist; analytics inventory requests “overall point differential” without a definition — **[SUPERSEDED IN PART by R-16, 2026-07-23; see "Reconciliation notes" after this table]** | Player-game; current value is exact for adjacent-placement definition, but may not match intended metric. Historical games suffice once defined; imports do not currently finalize | Requires a decision on opponent/table baseline and tie behavior before selecting joins. Define in Phase 2; expose Phase 7-13. Medium |
...
#### Reconciliation notes (owner rulings R-13/R-15/R-16, 2026-07-23)

These annotate the two capability-register rows above whose classification an owner
ruling has since overtaken. The original rows are **retained unedited**; only an
inline `[SUPERSEDED IN PART]` marker was appended to the affected cell. Authority:
`docs/redesign/DECISIONS.md` → "Phase 2 / analytics — owner rulings R-13–R-17". No
classification value was changed, and no analytics contract was reconciled here — that
is a separate work item.

- **"Overall point differential" — SUPERSEDED IN PART by R-16.** R-16 defines overall
  point differential as **two player-level metrics**, both scaled by player count:
  (a) score minus the **next-closest player**, and (b) score against the **field
  average**. This overtakes the row's "without a definition" / "Requires further
  verification" framing. The definition is **directional, not complete**: R-16 leaves
  **four open sub-questions** — analytics **Q-3** (which of the two is the ranking
  metric), **Q-4** (does the field average include the player's own score), **Q-5**
  (relationship to the existing sole-winner Win Point Differential), and **Q-6**
  (tied-first behaviour) — all recorded OPEN in `DECISIONS.md`. The row's join and
  tie-behaviour decision still awaits those answers.
- **"Opponent-adjusted performance" — SUPERSEDED IN PART; the three clauses resolve
  ...
  - **adjusted model** — **SPLIT per R-15.** The **opponent-adjusted margin**
    (raw/expected/adjusted, Phase 17.3) now **has an anchor** — ELO supplies a
    per-opponent rating — so that model stands, with a recorded dependency on Phase 7
    producing the rating before Phase 17 consumes it. The **Group-Chemistry
    expected-performance model** (Phase 18.5) is a **different object** (how players
    perform *together*, not a rating per player), has **no canonical definition**, and
    is **not** supplied by ELO, so for it the "no adjusted model exists" clause **still
    holds**. The two must not be conflated.
...
#### R-17 reconciliation note (CONTESTED, 2026-07-23)

Rows 11–14 above (Purchase conversion, Purchased share of hand acquisitions, Hand
utilization, End-hand carryover — the derived ratio family) are classified "Not
currently possible from recorded data" because their primitive inputs (metrics 1–10)
are recorded as unpersisted. Owner ruling **R-17** (`docs/redesign/DECISIONS.md` →
"R-17 — the six card-acquisition primitives") records a **contradicting** finding.
Both sides are set out here and neither is acted on; **the classification is NOT
changed.**

- **Contract side (retained, current):** the ratio family requires persisted
  purchase / seen / hand-entry primitives; the schema audit above finds none, so the
  ratios remain unavailable, not estimated.
- **R-17 side [PRIOR]:** an owner-supplied sample log examined 2026-07-23 showed
  **five of the six** R-17 primitives appear **recoverable from raw log content**
  ("kept N project cards"; "bought N card(s)" resolving per generation; named draw
  lines attributable to a player; "sold N patents"). If the primitives are
  recoverable, the ratio family's "not currently possible" classification is
  contestable.
- **Why CONTESTED, not corrected:** the finding is **[PRIOR]** from **one** log — one
  log is not the corpus — and whether raw game logs are stored immutably enough for a
  reparse to reach them is **open analytics question Q-8** (`DECISIONS.md`). Acting on
  it is the reconciliation / repository-read work item, not this one.
