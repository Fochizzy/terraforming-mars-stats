...
| `/cards` | `src/app/(app)/cards/page.tsx` | Canonical full Card Lookup browser | `/cards` | No | Auth + active group; middleware and `(app)` guard protect the route | `CardsPage`, `CardLookupBrowser`, detail dialog, shell/switcher | group context, full `cards` catalog, batched promo metadata, stored image fields | Completed post-Phase-2 preservation task; later Phase 13 analytics remains separate | Retain full catalog behavior; promo browsing may remain specialized only | Low |
| `/glossary` | `src/app/(app)/glossary/page.tsx` | Canonical fragment-addressable Glossary | `/glossary` | No | Auth through `(app)` and middleware | `GlossaryContent`, centralized registry and controlled rich-text renderer | static typed compatibility vocabulary and current semantic wording | Completed post-Phase-2 preservation task | Retain stable categories, entry slugs, and fragment behavior | Low |
...
| `/group/settings` | `src/app/(app)/group/settings/page.tsx` | Group name, analytics opt-in, defaults | Supporting Group Settings route, URL undecided | Yes if moved | Auth + active group; role semantics currently equal-member write | `GroupSettingsPage`, `GroupSettingsForm`, shell/switcher | group settings, promo sets, RLS | Phase 3 decision; Phase 10/20 implementation | Approved URL and actual owner/editor/viewer policy are tested | Medium |
...
| SetupStep | `src/features/games/log-game/setup-step.tsx` | `LogGameWizard` | Date, map, counts, promos, import link | retain | Log a Game setup | reference rows, RHF | parent wizard test | field validation/config coverage | 4 | Retain until redesigned equivalent passes | Low |
...
for generations, player count, drafting, corporation, Prelude, player
...
| Maps, promos, generation count | Available now | compatible-configuration filters and catalog version labels | drafting flag/config coverage is new | Existing recorded values usable; missing selections remain missing | 4/8-13 | zero relations can mean none or unrecorded |
| Gameplay expansion configuration | Retired in Phase 4, Step 4.2 | none; do not recreate a gameplay filter | none | Historical relation rows were intentionally removed; intrinsic catalog expansion metadata remains | retired | catalog metadata is not evidence that an expansion was played |
