...
| Canonical win-point differential | DERIVABLE | winner flags and final scores | two inconsistent SQL implementations today | Leaderboard and all outcome comparisons | central tie-aware formula and repository/view contract | View/RPC optional; no new raw fields | no non-winner makes margin unavailable; tied first needs explicit status | Show eligible winner-games and tie exclusions | 2 then 7-13 | tied-first value policy |
...
| Canonical win-point differential | Derivable | one tie-aware calculation/view and tests | optional canonical view/RPC; no new raw fact | Backfillable from finalized scores; tied first policy explicit | 2 | current view/snapshot formulas differ |
