...
returns table (
  group_id uuid, game_id uuid, played_on date, player_id uuid, player_name text,
  card_id uuid, card_name text, is_winner boolean, thumbnail_url text, full_image_url text
)
language sql stable security definer set search_path = ''
as $$
  select distinct
    g.group_id, g.id as game_id, g.played_on,
    resolved.player_id, resolved.player_name,
    e.card_id, c.card_name, gp.is_winner,
  ...
  join public.games g
    on g.id = gli.game_id and g.status = 'finalized'
  ...
  join lateral (
    ...
    from (
      ...
      where p.group_id = g.group_id
        and p.normalized_display_name = btrim(regexp_replace(lower(coalesce(e.payload->>'actor', '')), '[^a-z0-9]+', ' ', 'g'))
      union all
      ...
      where pia.group_id = g.group_id and pia.source_type = 'game_log'
        and pia.normalized_alias = btrim(regexp_replace(lower(coalesce(e.payload->>'actor', '')), '[^a-z0-9]+', ' ', 'g'))
    ) candidates
    order by candidates.preference limit 1
  ) resolved on true
  join public.game_players gp on gp.game_id = g.id and gp.player_id = resolved.player_id
  where e.event_type = 'card_played' and e.card_id is not null;
$$;
...
select group_id, game_id, played_on, player_id, player_name, card_id, card_name, is_winner, thumbnail_url, full_image_url
...
returns table (
  group_id uuid, game_id uuid, played_on date, player_id uuid, player_name text,
  tag_code text, tag_count integer, total_points integer, is_winner boolean,
  corporation_id uuid, corporation_name text
)
language sql stable security definer set search_path = ''
as $$
  select
    g.group_id, g.id as game_id, g.played_on,
    resolved.player_id, resolved.player_name,
    ts.tag_code, ts.tag_count, gp.total_points, gp.is_winner,
  ...
  join lateral (
    ...
    from (
      ...
      from public.player_import_aliases pia
      join public.players alias_player on alias_player.id = pia.player_id and alias_player.group_id = pia.group_id
    ...
    order by candidates.preference, candidates.player_name limit 1
  ) resolved on true
  join public.game_players gp on gp.game_id = g.id and gp.player_id = resolved.player_id
  left join lateral (
    select gpc.corporation_id from public.game_player_corporations gpc where gpc.game_player_id = gp.id
    union all
    select gp.corporation_id
    where gp.corporation_id is not null
      and not exists (select 1 from public.game_player_corporations gpc where gpc.game_player_id = gp.id)
...
with (security_invoker = true) as
select group_id, game_id, played_on, player_id, player_name, tag_code, tag_count, total_points, is_winner, corporation_id, corporation_name
