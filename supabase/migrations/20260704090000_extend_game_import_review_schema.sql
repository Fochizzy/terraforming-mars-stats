...
begin
  ...
  upserted_events as (
    ...
    select
      parsed_events.game_log_import_id,
      parsed_events.board_space,
      parsed_events.card_id,
      parsed_events.confidence_level,
      parsed_events.event_order,
      parsed_events.event_type,
      parsed_events.generation_number,
      parsed_events.line_classification,
      parsed_events.payload,
      parsed_events.raw_line,
      parsed_events.resource_amount,
      parsed_events.resource_type,
      parsed_events.tile_type
    from parsed_events
  ...
  deleted_events as (
    ...
    where gle.game_log_import_id = p_game_log_import_id
      and not exists (
        select 1
        from parsed_events
        where parsed_events.event_order = gle.event_order
      )
    returning gle.id
  )
  select gle.id, gle.event_order
