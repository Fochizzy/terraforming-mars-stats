...
corporation_entries as (
  select
    pr.id, pr.game_id, pr.player_id, corporation_selections.corporation_id,
    pr.placement, pr.is_winner, pr.total_points, pr.final_megacredits,
    pr.cities_points, pr.greenery_points, pr.card_points_total,
    pr.card_points_microbes, pr.card_points_animals, pr.card_points_jovian,
    pr.tr_points, pr.milestone_points, pr.award_points, pr.other_card_points,
    pr.created_at
  from player_results pr
  join lateral (
    select gpc.corporation_id from game_player_corporations gpc where gpc.game_player_id = pr.id
    union all
    select pr.corporation_id
    where pr.corporation_id is not null
      and not exists (select 1 from game_player_corporations gpc where gpc.game_player_id = pr.id)
  ) corporation_selections on true
),
milestone_counts as (
  select gm.winner_game_player_id as game_player_id, count(*) as milestones_won
  from game_milestones gm where gm.winner_game_player_id is not null group by 1
),
award_counts as (
  select ga.winner_game_player_id as game_player_id, count(*) as awards_won
  from game_awards ga where ga.place = 1 and ga.winner_game_player_id is not null group by 1
),
enriched as (
  select pr.*, coalesce(mc.milestones_won, 0) as milestones_won, coalesce(ac.awards_won, 0) as awards_won
  from player_results pr
...
corporation_enriched as (
  select ce.*, coalesce(mc.milestones_won, 0) as milestones_won, coalesce(ac.awards_won, 0) as awards_won
...
tag_summary_player_links as (
  select gli.game_id, resolved.player_id, ts.tag_code, ts.tag_count
  ...
  join lateral (
    ...
    from (
      select pia.player_id, 1 as preference from player_import_aliases pia
      where pia.group_id = g.group_id and pia.source_type = 'game_log' and pia.normalized_alias = ts.normalized_player_name
      union all
      select p.id as player_id, 2 as preference from players p
      where p.group_id = g.group_id and p.normalized_display_name = ts.normalized_player_name
    ) candidates order by candidates.preference limit 1
  ) resolved on true
),
corp_stats as (
  select c.name as corporation_name, count(*) as plays,
  ...
  from corporation_enriched ce join corporations c on c.id = ce.corporation_id group by c.name
),
prelude_base as (
  select e.*, pre.name as prelude_name from enriched e
  join game_player_preludes gpp on gpp.game_player_id = e.id
  join preludes pre on pre.id = gpp.prelude_id
),
prelude_stats as (
  select pb.prelude_name, count(*) as plays,
  ...
  from prelude_base pb group by pb.prelude_name
),
pair_stats as (
  select c.name as corporation_name, pre.name as prelude_name, count(*) as plays,
    round(avg(case when ce.is_winner then 1 else 0 end)::numeric, 3) as win_rate,
    round(avg(ce.total_points)::numeric, 1) as avg_points
...
corp_tag_stats as (
  select c.name as corporation_name, tsl.tag_code, round(avg(tsl.tag_count)::numeric, 1) as avg_tag_count
  from corporation_enriched ce
  join corporations c on c.id = ce.corporation_id
  join tag_summary_player_links tsl on tsl.game_id = ce.game_id and tsl.player_id = ce.player_id
  group by c.name, tsl.tag_code
),
award_funding as (
  select a.name as award_name, count(*) as funded_count,
    count(*) filter (where ga.place = 1 and ga.winner_game_player_id = ga.funded_by_game_player_id) as funder_won_count
  from game_awards ga
...
card_plays as (
  ...
  from enriched e
  join analytics.game_log_event_facts f on f.game_id = e.game_id and f.player_id = e.player_id
  where f.event_type = 'card_played' and f.card_id is not null
),
card_stats as (
  select c.card_name, count(*) as plays,
    round(avg(case when cp.is_winner then 1 else 0 end)::numeric, 3) as win_rate_when_played
  from card_plays cp join cards c on c.id = cp.card_id group by c.card_name
),
tag_win_stats as (
  select tsl.tag_code,
    ...
    count(*) as samples
  from enriched e
  join tag_summary_player_links tsl on tsl.game_id = e.game_id and tsl.player_id = e.player_id
  group by tsl.tag_code
),
baseline as (
  select round(avg(case when e.is_winner then 1 else 0 end)::numeric, 3) as win_rate from enriched e
)
select jsonb_build_object(
  'corporations', (select coalesce(jsonb_agg(to_jsonb(cs) order by cs.plays desc, cs.corporation_name), '[]'::jsonb) from corp_stats cs),
  'preludes', (select coalesce(jsonb_agg(to_jsonb(ps) order by ps.plays desc, ps.prelude_name), '[]'::jsonb) from prelude_stats ps),
  'pairs', (select coalesce(jsonb_agg(to_jsonb(pp) order by pp.plays desc, pp.corporation_name), '[]'::jsonb) from (select * from pair_stats order by plays desc limit 100) pp),
  'corporationTags', (select coalesce(jsonb_agg(to_jsonb(ct) order by ct.corporation_name, ct.avg_tag_count desc), '[]'::jsonb) from corp_tag_stats ct),
  'awardFunding', (select coalesce(jsonb_agg(to_jsonb(af) order by af.funded_count desc, af.award_name), '[]'::jsonb) from award_funding af),
  'cards', (select coalesce(jsonb_agg(to_jsonb(cd) order by cd.plays desc, cd.card_name), '[]'::jsonb) from (select * from card_stats order by plays desc limit 150) cd),
  'tagWins', (select coalesce(jsonb_agg(to_jsonb(tw) order by tw.tag_code), '[]'::jsonb) from tag_win_stats tw),
  'baselineWinRate', (select coalesce((select win_rate from baseline), 0)),
  'totalGames', (select count(distinct game_id) from player_results)
);
$function$;
