-- Add heat_actions to game_players so that temperature raises can be captured
-- directly from the OCR import flow (no game log required).
--
-- The column is nullable; NULL means "not recorded via import" (the existing
-- log-event path is unchanged and still the primary source).
--
-- get_global_insight_metrics is updated so that the terraforming_rows CTE
-- uses the stored heat_actions value as the authoritative heat count when it
-- is present, falling back to counting log events as before.

alter table public.game_players
  add column if not exists heat_actions integer;

-- Re-create get_global_insight_metrics with the merged heat source.
-- Only the terraforming_rows CTE changes; all other CTEs are identical to
-- 20260715182325_fix_heat_actions_in_global_insight_metrics.sql.
...
with player_results as (
  select
    ...
    gp.total_points,
    gp.tr_points,
    -- Stored heat_actions from the import form (null = not recorded this way)
    gp.heat_actions as stored_heat_actions
  from games g
...
-- Per-player log-event counts aggregated for the terraforming share table.
-- stored_heat_actions from game_players is used as the authoritative heat
-- count when present; otherwise the log-event count is used.
log_heat_by_player as (
  select
    rle_heat.player_id,
    count(*)::int as log_heat_actions
  from resolved_log_events rle_heat
  where rle_heat.player_id is not null
    and rle_heat.event_type = 'global_parameter_changed'
    and lower(coalesce(
      rle_heat.payload->>'parameterType',
      rle_heat.payload->>'parameter',
      rle_heat.resource_type,
      ''
    )) in ('temperature', 'heat')
  group by rle_heat.player_id
),
terraforming_rows as (
  select
    ...
    -- Use the stored import value when available, otherwise count log events.
    coalesce(
      max(pr.stored_heat_actions),
      max(lh.log_heat_actions)::int,
      (count(*) filter (where te.parameter_type = 'heat'))::int
    ) as heat_actions,
  ...
  left join player_results pr
    on pr.player_id = te.player_id
  left join log_heat_by_player lh
    on lh.player_id = te.player_id
